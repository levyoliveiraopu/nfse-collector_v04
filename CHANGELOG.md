...
- INFRA-01: runbook de hardening da VPS Hostinger em `infra/vps-hardening.md` (usuario `deploy`, SSH chave-only, `PermitRootLogin no`, UFW 22/80/443, fail2ban jail SSH, unattended-upgrades, TZ `America/Sao_Paulo` com `systemd-timesyncd`); execucao na VPS fica a cargo do owner.
